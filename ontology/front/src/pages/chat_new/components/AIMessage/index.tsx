...
const AITestCaseResultCard = (props:any) => {
 ...
 // const testCases = props.testCases;
  ...
  const { onRunCase, onCreateTestCaseReportTab, reportDisabled } = props;
  ...
  const viewResutTable = () => {
    if (isBatch) {
      // 如果提供了创建测试报告 tab 的回调，则创建 tab 页，否则打开 Modal
      if (onCreateTestCaseReportTab) {
        onCreateTestCaseReportTab(sessionId, total);
      } else {
          ...
          <div className="progress-head">
            {
              isBatch ?
                <><div>批量测试执行中</div>
                  <Button type='outline' disabled={reportDisabled} onClick={viewResutTable}>测试报告</Button>
                </>
                : <><div>测试执行中</div>
...
const AIMessage: React.FC<AIMessageProps> = ({ message, onRunCase, siderOntologyData, ontology, afterReRunTestCase, onCreateTestCaseReportTab, reportDisabled }) => {
  // 判断是否正在流式传输（用于显示光标）
      ...
      <div className="ai-content-body">
        {message && message.content ? message.content?.map((content: any) => {
          return content.type == 'answer' ? <MarkdownRender key={content.id} content={content.data} /> : content.type == 'mcp' ?
            <McpPanel key={content.id} data={content.data} /> : content.type == 'test-case-result' ? <AITestCaseResultCard key={content.id} sessionId={content.id} content={content} testCases={content.data} onRunCase={onRunCase} ontology={ontology} siderOntologyData={siderOntologyData} afterReRunTestCase={afterReRunTestCase} onCreateTestCaseReportTab={onCreateTestCaseReportTab} reportDisabled={reportDisabled}/> : ''
        }) : (
