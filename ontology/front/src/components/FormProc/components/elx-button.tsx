    ...
    return (
        <div className='elx-button'>
            <FormItem  {...item} label={label} field={field}>
                ...
                <IconTable />
                {options.title || loginT('自动建表')}
                </Button>
            </FormItem>
            <Modal
                title={options.title || loginT('自动建表')}
                style={{
                    width: '70%',
                    cursor: 'move'
                }}
                ...
                {/* <Spin  loading={loading}> */}
                    ...
                    <Checkbox checked={dsProfile} onChange={(value)=>onChecChange(value)}>{options.checkboxLabel||loginT('生产环境建表')}</Checkbox>
                    {/* </Spin> */}
                </div>
        </Modal>
        </div>
