...
import Option from '@arco-design/web-react/es/Cascader/panel/option';
import { isFunction, isObject } from '@arco-design/web-react/es/_util/is';
import { CascaderPanelProps, OptionProps } from '@arco-design/web-react/es/Cascader/interface';
...
import { ArrowDown, Esc, Enter, ArrowUp, ArrowRight, ArrowLeft } from '@arco-design/web-react/es/_util/keycode';
import useUpdate from '@arco-design/web-react/es/_util/hooks/useUpdate';
import Node from '@arco-design/web-react/es/Cascader/base/node';
import { getMultipleCheckValue } from '@arco-design/web-react/es/Cascader/util';
import VirtualList from '@arco-design/web-react/es/_class/VirtualList';
...
const ListPanel = <T extends OptionProps>(props: CascaderPanelProps<T>) => {
  ...
  const getChildren = (lvl, pathNodes, list) => {
    let currentNode = null;
    if (pathNodes[lvl]) {
      const children = pathNodes[lvl].children;
      if (children) {
      ...
      if (currentMenu.length > 0 && currentMenu[0] && currentMenu[0].children) {
        const children = currentMenu[0].children;
        if (children) {
          currentNode = children.length > 0 && children[0]
          list.push(currentMenu[0].children)
        }
      }
    }
    if (currentNode && currentNode.children) {
      lvl += 1;
      getChildren(lvl, pathNodes, list);
    }
  }
