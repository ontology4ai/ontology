...
Configurations are fetched lazily from remote config service when creating services.
This supports config hot-reload without service restart.
"""
...
class DatabaseServiceFactory:
    ...
    Configurations are fetched on-demand from remote config service.
    """
    
    def __init__(self):
        """Initialize the factory (no config loading at startup)"""
        logger.info("Database service factory initialized (lazy config loading)")
    
    async def create_mysql_service(self, config_override: Optional[dict] = None) -> Optional[MySQLService]:
        """
        Create MySQL service instance using configuration fetched on-demand.
    ...
    async def create_postgres_service(self, config_override: Optional[dict] = None) -> Optional[PostgresORMService]:
        """
        Create Postgres ORM service instance using configuration fetched on-demand.

        Args:
            config_override: Optional configuration overrides

        Returns:
            PostgresORMService instance or None if not configured
        """
        try:
            config = await get_postgres_config()
            if not config:
                logger.warning("Postgres not configured, skipping service creation")
                return None

            if config_override:
                config.update(config_override)

            # Backward-compat: accept both default_schema and schema (schema wins if provided)
            default_schema = config.get('default_schema')
            legacy_schema = config.get('schema')
            resolved_schema = legacy_schema if legacy_schema is not None else default_schema
            ...
            service = PostgresORMService(**params)
            logger.info(f"Postgres service created: {service}")
            return service

        except Exception as e:
            logger.error(f"Failed to create Postgres service: {e}")
            logger.error(traceback.format_exc())
            raise
    
    async def create_all_services(self) -> dict:
        """
        ...
        mysql_service = await self.create_mysql_service()
        if mysql_service:
            services['mysql'] = mysql_service
        
        # Create Postgres service
        postgres_service = await self.create_postgres_service()
        if postgres_service:
...
# Async convenience functions for creating individual services

async def create_mysql_service(config_override: Optional[dict] = None) -> Optional[MySQLService]:
    """Create MySQL service using configuration fetched on-demand"""
    factory = get_database_factory()
    return await factory.create_mysql_service(config_override)


async def create_postgres_service(config_override: Optional[dict] = None) -> Optional[PostgresORMService]:
    """Create Postgres service using configuration fetched on-demand"""
    factory = get_database_factory()
    return await factory.create_postgres_service(config_override)


async def create_all_database_services() -> dict:
    """Create all configured database services"""
    factory = get_database_factory()
    return await factory.create_all_services()
